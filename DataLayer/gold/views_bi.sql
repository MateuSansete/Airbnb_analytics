...
-- SCHEMA BI - VIEWS ANALÍTICAS PARA BUSINESS INTELLIGENCE


-- Criação do schema BI
CREATE SCHEMA IF NOT EXISTS bi;



-- 1. VW_KPI_GERAL - VISÃO CONSOLIDADA DO NEGÓCIO

-- Objetivo: Dashboard executivo com principais indicadores operacionais

CREATE OR REPLACE VIEW bi.VW_KPI_GERAL AS
SELECT 
...
-- 2. VW_ANALISE_YEAR_OVER_YEAR - CRESCIMENTO E VARIAÇÃO ANUAL

-- Objetivo: Identificar tendências de crescimento/declínio com variações percentuais

CREATE OR REPLACE VIEW bi.VW_ANALISE_YEAR_OVER_YEAR AS
...
-- 3. VW_TOP_ROTAS_MOVIMENTADAS - TOP 10 ROTAS MAIS MOVIMENTADAS

-- Objetivo: Identificar principais operações e sua performance (para análise de rotas críticas)

CREATE OR REPLACE VIEW bi.VW_TOP_ROTAS_MOVIMENTADAS AS
...
-- 4. VW_RANKING_COMPANHIAS_PONTUALIDADE - RANKING DE COMPANHIAS AÉREAS

-- Objetivo: Identificar melhores e piores companhias (para gráfico de barras)

CREATE OR REPLACE VIEW bi.VW_RANKING_COMPANHIAS_PONTUALIDADE AS
...
-- 5. VW_AEROPORTOS_PROBLEMATICOS - TOP 20 AEROPORTOS MAIS PROBLEMÁTICOS

-- Objetivo: Identificar aeroportos com maiores problemas (para matriz de performance)

CREATE OR REPLACE VIEW bi.VW_AEROPORTOS_PROBLEMATICOS AS
...
-- 6. VW_SEVERIDADE_ATRASOS - DISTRIBUIÇÃO DE SEVERIDADE DE ATRASOS

-- Objetivo: Classificar voos por faixas de atraso para análise de distribuição

CREATE OR REPLACE VIEW bi.VW_SEVERIDADE_ATRASOS AS
...
-- 7. VW_CAUSAS_ATRASOS - BREAKDOWN PERCENTUAL DE CAUSAS

-- Objetivo: Contribuição de cada causa (para gráfico de pizza/donut)

CREATE OR REPLACE VIEW bi.VW_CAUSAS_ATRASOS AS
...
-- 8. VW_SAZONALIDADE_MENSAL - PADRÕES SAZONAIS E CAUSAS POR MÊS

-- Objetivo: Identificar padrões sazonais e principais causas por mês

CREATE OR REPLACE VIEW bi.VW_SAZONALIDADE_MENSAL AS
...
-- 9. VW_MATRIZ_ANO_MES - HEATMAP ANO × MÊS COM RANKING

-- Objetivo: Heatmap ano-mês mostrando evolução mensal ao longo dos anos

CREATE OR REPLACE VIEW bi.VW_MATRIZ_ANO_MES AS
...
-- 10. VW_COMPARACAO_PANDEMIA - IMPACTO COVID-19

-- Objetivo: Medir impacto da COVID-19 na operação (análise comparativa)

CREATE OR REPLACE VIEW bi.VW_COMPARACAO_PANDEMIA AS
...
-- CONSULTAS PARA TESTAR AS VIEWS

-- SELECT * FROM bi.VW_KPI_GERAL;
-- SELECT * FROM bi.VW_ANALISE_YEAR_OVER_YEAR;
-- SELECT * FROM bi.VW_TOP_ROTAS_MOVIMENTADAS;
-- SELECT * FROM bi.VW_RANKING_COMPANHIAS_PONTUALIDADE;
-- SELECT * FROM bi.VW_AEROPORTOS_PROBLEMATICOS;
-- SELECT * FROM bi.VW_SEVERIDADE_ATRASOS;
-- SELECT * FROM bi.VW_CAUSAS_ATRASOS;
-- SELECT * FROM bi.VW_SAZONALIDADE_MENSAL;
-- SELECT * FROM bi.VW_MATRIZ_ANO_MES;
-- SELECT * FROM bi.VW_COMPARACAO_PANDEMIA;
